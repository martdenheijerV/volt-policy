...
--
-- EU-pure auth migration. Removes the dependency on the managed-
-- platform `auth.users` table and the auth.uid() / auth.role()
-- helpers. After this migration, identity comes from an external
-- OIDC provider (Volt Auth / Authentik) and is upserted into
-- `public.profiles` keyed by `oidc_sub`.
--
