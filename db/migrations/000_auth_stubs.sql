...
-- a managed-platform `auth` schema (auth.users, auth.uid(),
-- auth.role()). On plain self-hosted Postgres that schema doesn't
-- exist, so 001-004 would fail before migration 005 has a chance to
-- rewire them.
--
